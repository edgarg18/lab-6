...
void EduBase_LCD_Ports_Init(void)
{
	...
	// RO bit (Bit0 )) in the RCGCGPIO register
	SYSCTL->RCGCGPIO |= 0x01;
	
	// Configure the PA5, PA4, PA3, PA2 pins as output
	// by setting Bits 5 to 2 in the DIR register
	GPIOA->DIR |= 0x3C;
	
	// Configure the PA5, PA4, PA3, PA2 pins to function as
	// GPIO pins by clearing bits 5 to 2 in the AFSEL register
	GPIOA->AFSEL &= ~0x3C;
	
	// Enable the digital functionality for the PA5, PA4, PA3, PA2 pins
	// by setting Bits 5 to 2 in the DEN register 
	GPIOA->DEN |= 0x3C;
	
	// Initialize the Output of the PA5, PA4, PA3, PA2 pins to zero
	...
	// Configure the PC6 pin as output by setting Bit6 in the DIR register
	GPIOC->DIR |= 0x40;
	...
	// by setting the Bit 6 in the DEN register
	GPIOC->DEN |= 0x40;
	
	// Initialize the output of the PC6 pin to zero
	// by clearing the Bit 6 in the DATA register 
	GPIOC->DATA &= ~0x40;
